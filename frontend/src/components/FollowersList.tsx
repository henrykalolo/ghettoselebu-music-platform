...
const FollowersList: React.FC = () => {
  ...
  const fetchFollowers = async () => {
    try {
      const response = await apiService.socialAPI.getFollowers(state.profile?.id || 0);
  ...
  const handleFollow = async (userId: number) => {
    ...
    try {
      await apiService.socialAPI.followUser(userId);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">People who follow you</p>
        </div>
      </div>
      ...
      {filteredFollowers.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">
            {searchTerm ? 'No matching users found' : "No followers yet"}
          </h3>
            ...
            <div key={user.id} className="bg-gray-800 rounded-lg p-4 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className={`flex items-center space-x-1 px-3 py-1 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                    user.is_following 
